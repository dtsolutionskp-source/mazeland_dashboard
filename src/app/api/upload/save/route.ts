...
export async function POST(request: NextRequest) {
  try {
    ...
    const { dailyData, channels, categories, summary, year, month } = body

    console.log('[Upload Save] Saving data for:', year, month)
    console.log('[Upload Save] Daily data count:', dailyData?.length)

    // 일별 데이터에서 합계 재계산
    const totalOnline = dailyData.reduce((sum: number, d: DailyData) => sum + d.online, 0)
    const totalOffline = dailyData.reduce((sum: number, d: DailyData) => sum + d.offline, 0)
    ...
    const onlineSales = Object.entries(channels || {}).map(([code, data]: [string, any]) => ({
    ...
    // 시작일/종료일 계산
    const dates = dailyData.map((d: DailyData) => new Date(d.date)).sort((a: Date, b: Date) => a.getTime() - b.getTime())
    ...
    for (const [code, data] of Object.entries(channels || {}) as [string, ChannelData][]) {
    ...
    const offlineRevenue = totalOffline * BASE_PRICE
    ...
    const storedData: StoredUploadData = {
      ...
      fileName: `${year}년_${month}월_수정데이터.xlsx`,
      ...
      dailyData: dailyData.map((d: DailyData) => ({
        date: d.date,
        online: d.online,
        offline: d.offline,
      ...
      monthly: {
        onlineByChannel: Object.fromEntries(
          Object.entries(channels || {}).map(([code, data]: [string, any]) => [code, data.count || 0])
        ),
        onlineByAge: {},
        offlineByCategory: Object.fromEntries(
          Object.entries(categories || {}).map(([code, data]: [string, any]) => [code, data.count || 0])
        ),
        revenue: {
          online: totalOnline * BASE_PRICE,
          onlineFee: Math.round(totalOnline * BASE_PRICE - onlineNetRevenue),
          onlineNet: Math.round(onlineNetRevenue),
          offline: offlineRevenue,
          total: totalCount * BASE_PRICE,
    ...
    console.log('[Upload Save] Saved to upload-data.json')
    ...
    const channelCodes = Object.keys(channels || {})
    ...
    const sortedDailyData = [...dailyData].sort((a: DailyData, b: DailyData) => 
    ...
    const dailyAggDataList: DailyAggData[] = sortedDailyData.map((d: DailyData, idx: number) => {
      ...
      const dayRatioOnline = totalOnline > 0 ? d.online / totalOnline : 0
      const dayRatioOffline = totalOffline > 0 ? d.offline / totalOffline : 0

      const channelSales: DailyChannelSale[] = channelCodes.map(code => {
        const chData = (channels as Record<string, ChannelData>)[code]
      ...
      const categorySales: DailyCategorySale[] = categoryCodes.map(code => {
        const catData = (categories as Record<string, CategoryData>)[code]
